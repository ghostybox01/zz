...
export default function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setLists((prev) => {
          // Server is the sole source of truth. Only keep prev entries
          // whose id the server reports as present; discard everything
          // else (stale localStorage, lists deleted server-side, etc.).
  ...
  const hitInsights = useMemo(() => {
    const total = reconStats.raw?.total_hits ?? findings.length
    const cracks = reconStats.raw?.total_valid ?? findings.filter((f) => f.severity === 'critical' || f.severity === 'high').length
    return { total, cracks }
  }, [reconStats.raw, findings])
