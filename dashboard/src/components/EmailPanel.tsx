...
const ALL_PROVIDERS = [
  ...
  'SparkPost', 'Mailtrap', 'Mailjet', 'SMTP', 'XSMTP',
...
function copyToClipboard(text: string, onToast: Props['onToast']) {
  const fallback = () => {
    try {
      ...
      onToast('Copied')
    } catch { onToast('Copy failed', 'Clipboard denied', 'error') }
  }
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(() => onToast('Copied')).catch(fallback)
  } else fallback()
}

function parseVerifyMeta(raw: string | null): Record<string, string> {
  if (!raw) return {}
  try {
    const parsed = JSON.parse(raw)
    if (parsed?.extra && Array.isArray(parsed.extra)) {
      return Object.fromEntries(parsed.extra.map((e: { key: string; value: string }) => [e.key, e.value]))
    }
    return parsed as Record<string, string>
  } catch { return {} }
}

function RecheckExtra({ raw }: { raw: string | null }) {
  const meta = parseVerifyMeta(raw)
...
export function EmailPanel({ onToast }: Props) {
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const r = await findingsApi.listEmail()
      setItems(r.findings)
    } catch (e) {
      onToast('Could not load email findings', (e as Error).message, 'error')
    } finally {
      setLoading(false)
    }
  }, [onToast])
  ...
  return (
    <div className="findings-panel">
      <header className="findings-panel__head">
        <div>
          <h2 className="findings-panel__title">Email &amp; Messaging — all providers</h2>
          <p className="muted findings-panel__sub">
            SMTP credentials, transactional email APIs, and SMS providers discovered by the scanner.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.25rem', marginBottom: '.75rem' }}>
        <button
          type="button"
          className={`btn-glass btn-glass--xs${filter === 'all' ? ' btn-glass--on' : ''}`}
      ...
      {!loading && visible.length === 0 && (
        <div className="muted-callout">
          <p style={{ margin: 0, fontWeight: 600 }}>No {filter === 'all' ? 'email/messaging' : filter} credentials yet</p>
          <p className="muted" style={{ margin: '.35rem 0 0', fontSize: '.82rem' }}>
            Enable the relevant scanner addon and start a crack session. Validated credentials appear here automatically.
          </p>
        </div>
      )}
                  ...
                  <tr key={item.id} style={isValid ? { background: 'rgba(0,255,100,0.04)' } : undefined}>
                    <td>
                      <span style={{
                        ...
                        fontSize: '.78rem', fontWeight: 600, color,
                      }}>
                    ...
                    <td>
                      ...
                      {item.metadata && (
                        <div className="muted" style={{ fontSize: '.68rem' }}>{item.metadata.slice(0, 50)}</div>
                      )}
                      <RecheckExtra raw={verifyRaw} />
                    </td>
                    <td>
