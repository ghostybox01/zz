...
type PanelTab = 'cPanel' | 'FTP' | 'WordPress'

const PANEL_TABS: PanelTab[] = ['cPanel', 'FTP', 'WordPress']
...
/** Parse credential fields from the stored key_value.
 *  FTP format:    host:port:user:pass
 *  cPanel format: host:user:pass
 *  WordPress:     host:user:pass
 */
function parseWebPanelDetail(item: DiscoveredKey, tab: PanelTab): {
  host: string
  port: string
  user: string
  pass: string
  ...
  return { host: parts[0] ?? '', port: '', user: parts[1] ?? '', pass: parts[2] ?? '' }
}
...
export function WebPanelsPanel({ onToast }: Props) {
  const [items, setItems] = useState<DiscoveredKey[]>([])
  const [loading, setLoading] = useState(true)
  const [activeTab, setActiveTab] = useState<PanelTab>('cPanel')

  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const r = await findingsApi.listWebPanels()
      setItems(r.findings)
    } catch (e) {
      onToast('Could not load web panel findings', (e as Error).message, 'error')
    } finally {
      setLoading(false)
    }
  }, [onToast])
  ...
  const remove = useCallback(async (id: number) => {
    if (!window.confirm('Delete this web panel finding?')) return
    try {
  ...
  return (
    <div className="findings-panel">
      <header className="findings-panel__head">
        <div>
          <h2 className="findings-panel__title">Web Panels — discovered credentials</h2>
          <p className="muted findings-panel__sub">
            cPanel, FTP, and WordPress credentials extracted and validated by the scanner.
          </p>
        </div>
      ...
      {loading && items.length === 0 && (
        <p className="muted-callout">Loading web panel findings…</p>
      )}
      ...
      {tabItems.length > 0 && (
        <div className="findings-table-wrap">
          <table className="findings-table">
            ...
            <tbody>
              {tabItems.map((item) => {
                const d = parseWebPanelDetail(item, activeTab)
                ...
                return (
                  ...
                  >
                    <td><code className="findings-key">{d.host || item.source_url}</code></td>
                    {activeTab === 'FTP' && (
                      <td className="muted" style={{ fontSize: '.75rem' }}>{d.port || '21'}</td>
                    )}
                    <td><code>{d.user || '—'}</code></td>
                    <td><code className="muted">{maskPass(d.pass) || '—'}</code></td>
                    <td>
